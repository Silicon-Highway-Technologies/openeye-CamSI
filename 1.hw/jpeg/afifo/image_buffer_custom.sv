...
always @(posedge clock_in) if(clock_enable_in) begin
    ...
    read_data_out <= mem[read_address_in]; //Enable Output Register = False
end

endmodule

module image_buffer_custom (
    input logic clock_in,
...
assign write_address = write_address_in[15:2];
assign read_address = read_address_in[15:2];
...
// Large RAM
inferred_lram_custom inferred_lram (
    .clock_in(clock_in),
